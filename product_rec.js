document.addEventListener('DOMContentLoaded', function(event) {
    ...
    .then(response => {
        ...
        if (response.products && Array.isArray(response.products) && response.products.length > 0) {
            ...
            if (productData.categoriesPaths && Array.isArray(productData.categoriesPaths)) {
    ...
    function fetchCompatibleCategories(compatibleCategoryIds) {
        console.log(`Rozpoczynam pobieranie produktów dla unikalnych kompatybilnych kategorii: ${compatibleCategoryIds}`);
    ...
    function fetchCategoryAndProducts(categoryId) {
        ...
        if (category) {
            ...
            if (category.products && category.products.length > 0) {
                console.log(`Liczba produktów w kategorii ${categoryId}: ${category.products.length}`);
                
                // Tworzymy zapytanie fetch dla danej kategorii
                fetch('/ajax.php?p=conversion&f=categories_info&ssl=1', {
                    method: 'POST',
                    body: JSON.stringify({ categories: [{ category_id: categoryId, products: category.products }] }),
                    credentials: 'same-origin',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
                ...
                .then(response => {
                    console.log(`Odpowiedź serwera dla kategorii ${categoryId}:`, response);
                ...
                .catch(error => {
                    console.error(`Błąd podczas pobierania danych dla kategorii ${categoryId}:`, error);
                });
            } else {
            ...
            data.categories.forEach(category => {
                ...
                if (category.products && category.products.length > 0) {
                    ...
                    randomProducts.forEach(product => {
                        ...
                        if (product.products_id !== productId && !displayedProducts.has(product.products_id)) {
